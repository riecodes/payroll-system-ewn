            ...
            <div class="form-group">
              <form method="POST" action="">
                  <div class="box-body shadow">
                      <div class="col-sm-4">
                          <label for="add-schedule" class="control-label">Filter by date</label>
                          <input type="text" id="datepicker_add" class="form-control" name="filter_date">
                      </div>
                      <div class="col-sm-4">
                          <label for="add-schedule" class="control-label">Filter by employee id</label>
                          <input type="text" id="filter-employee" class="form-control" name="filter_emp">
                      </div>
                      <div class="col-sm-4">
                        <label for="filter-location" class="control-label">Filter by location</label>
                          <select class="form-control" id="filter-location"  name="filter_loc">
                            <option value="" selected>-select-</option>
                            <?php
                              $sql = "SELECT * FROM location";
                              $query = $conn->query($sql);
                    ...
                    <!-- filter button -->
                    <div class="col-sm-3">
                      <input type="submit" id="filter-employee" class="col-sm-6 btn btn-info form-control" name="filter" value="filter">
                    </div>
              </form>
            </div>
                ...
                <tbody>
                  ...
                  if(!isset($_POST['filter'])){
                    ...
                    while($row = $query->fetch_assoc()){
                      echo "
                        <tr>
                        <td><input type='checkbox' class='employeeCheckboxUnassign' id='toUnassign' value='".$row['ass_id']."'>
                    ...
                    if(!isset($_POST['filter_date']) && !isset($_POST['filter_emp']) && !isset($_POST['filter_loc'])) {
                      echo "<h1>Not working</h1>";
                    } else {
                        // Code to execute when form is submitted and filters are set
                        $filterDate = $_POST['filter_date'];
                        $filterEmp = $_POST['filter_emp'];
                        $filterLoc = $_POST['filter_loc'];
                    }
                    ...
                    LEFT JOIN schedules ON ass_sched_fin.ass_schedule = schedules.id
                    LEFT JOIN employees ON ass_sched_fin.ass_employee_id = employees.id
                    WHERE ass_schedule = '$filterDate' OR ass_employee_id_sc = '$filterEmp' OR ass_location = '$filterLoc'";
                    $query = $conn->query($sql);
                    while($row = $query->fetch_assoc()){
                      echo "
                        <tr>
